...
def render_img(cfg):
    ...
    motionFill.load_state_dict(torch.load(cfg.mnet_path, map_location=torch.device('cpu')))
    motionFill.eval()
    ...
    o_data = parse_npz(cfg.object_path)
    bps_dists = torch.tensor(o_data['bps_dists'].reshape(1, 1024))
    object_transl = torch.tensor(o_data['obj_transl'].reshape(1, 3))
    ...
    fullpose_rotmat_gnet = results['fullpose_rotmat'].reshape(55, 3, 3)
    fullpose6D_gnet = fullpose_rotmat_gnet[:, :, :2]
    fullpose6D_gnet = fullpose6D_gnet.reshape(55 * 6).detach().numpy()
    ...
    I_cond[-1, :L, 0] = fullpose6D_gnet.flatten()
    ...
    if cfg.test > 0:
        ...
        fullpose_6D[0] = fullpose_6D_I[0]
        fullpose_6D[-1] = fullpose6D_gnet
    else:
...
if __name__ == '__main__':
    ...
    parser.add_argument('--gnet-path', required=True, type=str,
                        help='Path to the trained model')
    parser.add_argument('--mnet-path', required=True, type=str,
                        help='Path to the trained model')
    parser.add_argument('--object-path', required=True, type=str,
                        help='Path to the object data that gives the location of the object to test with gnet')
    ...
    parser.add_argument('--smooth', default=True, type=int)

    args = parser.parse_args()
    data_path = args.data_path
    gnet_path = args.gnet_path
